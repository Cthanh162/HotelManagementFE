      ...
      <div class="col-md-3">
        <div class="card p-3">
          ...
          <div class="mb-3">
            ...
            <input type="date" v-model="search.checkin" class="form-control" :min="minDate" />
          </div>

          <div class="mb-3">
            <label class="form-label">Ngày trả phòng</label>
            <input type="date" v-model="search.checkout" class="form-control" :min="search.checkin || minDate" />
          </div>
        ...
        <div class="row g-4">
          <div v-for="room in paginatedRooms" :key="room.roomId" class="col-12">
            <div class="card p-3 d-flex flex-row align-items-center justify-content-between flex-wrap" style="min-height: 180px;">
              <img :src="room.roomImages?.[0]?.url || room.roomImages?.[0]" alt="Room" width="180" height="120" class="me-3 rounded object-fit-cover" />
...
const minDate = ref(now.toISOString().split("T")[0]);
const minDateTime = ref(getCurrentMinDateTime());

const search = ref({
  q: '', roomType: '', capacity: '', minPrice: '', maxPrice: '',
...
watch(() => search.value.checkin, (val) => {
  ...
  minDateTime.value = isToday ? getCurrentMinDateTime() : selectedDate.toISOString().slice(0, 16);
});
